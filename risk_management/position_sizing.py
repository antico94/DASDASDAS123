...
class PositionSizer:
    ...
    def calculate_position_size(self, symbol, entry_price, stop_loss_price, atr_value=None):
        ...
        if entry_price is None or stop_loss_price is None:
            ...
            DBLogger.log_error("PositionSizer", error_msg)
        ...
        try:
            ...
            DBLogger.log_event("DEBUG", f"Risk amount: {risk_amount} ({self.max_risk_percent}% of {account_balance})", "PositionSizer")
            ...
            DBLogger.log_error("PositionSizer", f"Error calculating position size", exception=e)
    ...
    def calculate_atr_based_stop(self, current_price, atr_value, order_type, multiplier=1.5):
        ...
        """
        if atr_value <= 0:
            DBLogger.log_event("WARNING", f"Invalid ATR value: {atr_value}", "PositionSizer")
    ...
    def validate_position_size(self, symbol, position_size):
        ...
        if not symbol_info:
            DBLogger.log_event("WARNING", f"Could not get symbol info for {symbol}", "PositionSizer")
            return False
        ...
        if position_size < min_lot:
            DBLogger.log_event("WARNING", f"Position size {position_size} is below minimum {min_lot} for {symbol}", "PositionSizer")
            return False

        if position_size > max_lot:
            DBLogger.log_event("WARNING", f"Position size {position_size} exceeds maximum {max_lot} for {symbol}", "PositionSizer")
            return False

        # Check lot step - commented out due to floating point issues
